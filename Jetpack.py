...
EXTENSION_NAME = "Jetpack"
COLUMNS = ["Payload","Status Code", "Length (B)", "Body (B)"]

def payloader(target):
        ...
        for i in testcase_payload:
            header["alg"]=i
            copy=str(header)
            copy = copy.replace("u'","'")
            copy = copy.replace("'","\"")
            print(copy)
            c=base64.b64encode(copy.encode('ascii'))
            c=str(c.decode("ascii"))
            new_token=c[:-1]+"."+x[1]+"."+x[2]
            print(new_token)
...
class BurpExtender(
    ...
    def _makeHttpRequests(self):
        ...
        for payload in self._payloads:
            ...
            rowData = [payload, statusCode,len(response.getResponse()), content_length]
    ...
    def _updateClassFromUI(self):
        ...
        self._payloads = payloader(self._tokenTextField.text)
    ...
    def _constructAttackPanel(self, insets, messageEditorComponent):
        ...
        tokenLabel = JLabel("Token:")
        tokenLabelConstraints = GridBagConstraints()
        tokenLabelConstraints.gridx = 0
        tokenLabelConstraints.gridy = 3
        tokenLabelConstraints.anchor = GridBagConstraints.LINE_START
        tokenLabelConstraints.insets = insets
        attackPanel.add(tokenLabel, tokenLabelConstraints)

        self._tokenTextField = JTextField(25)
        self._tokenTextField.setMinimumSize(
            self._tokenTextField.getPreferredSize())
        tokenTextFieldConstraints = GridBagConstraints()
        tokenTextFieldConstraints.gridx = 1
        tokenTextFieldConstraints.gridy = 3
        tokenTextFieldConstraints.gridwidth = 2
        tokenTextFieldConstraints.anchor = GridBagConstraints.LINE_START
        tokenTextFieldConstraints.insets = insets
        attackPanel.add(self._tokenTextField, tokenTextFieldConstraints)
        ...
        protocolCheckBoxConstraints.gridy = 2
        protocolCheckBoxConstraints.gridwidth = 3
        protocolCheckBoxConstraints.anchor = GridBagConstraints.CENTER
